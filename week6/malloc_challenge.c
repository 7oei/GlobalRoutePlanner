...
// Best fitのため、サイズ順に並べたfree_listを作る
// free_listの適切な位置にadd
void my_add_to_free_list(simple_metadata_t *metadata)
{
  assert(!metadata->next);
  simple_metadata_t *cur = simple_heap.free_head; //free_listの最初のmetadata
  if (cur->size > metadata->size)
  {
    // metadataをheadに追加(sizeが最小)
    metadata->next = simple_heap.free_head;
    simple_heap.free_head = metadata;
  }
  else
  {
    simple_metadata_t *prev;
    while (cur->size < metadata->size && cur->next != NULL)
    {
      prev = cur;
      cur = cur->next;
    }
...
// 追加方法をsample_add_to_free_listからamy_add_to_free_listにするだけで他はsampleと一緒
void *my_malloc(size_t size)
{
  simple_metadata_t *metadata = simple_heap.free_head;
  simple_metadata_t *prev = NULL;
  // Find the first free slot the object fits.
  // free_listがサイズ順になっているからfirst-fitかつbest-fit
  while (metadata && metadata->size < size)
  {
    prev = metadata;
    metadata = metadata->next;
